...
- [ ] **Task 1: Install Convex Auth prerequisite (shared with 2.1) (AC: #1, #3)**
  - [ ] If not already added by Story 2.1: `pnpm --filter @sommycomfort/backend add @convex-dev/auth @auth/core` (pin `@auth/core` to the version `@convex-dev/auth`'s README specifies), then `npx @convex-dev/auth` to generate `convex/auth.config.ts` + `SITE_URL`/`JWKS`/`JWT_PRIVATE_KEY` env. Verify the installed `convexAuth`/`getAuthUserId`/`authTables` signatures against `node_modules/.pnpm/@convex-dev+auth@*/` (APIs evolved across 0.0.x). [Convex guide §Auth&RBAC.1, Gotchas]
