...
- **Live Prisma connect + first migration (no Docker/Postgres here).** The schema + generated client ship; applying a migration to a real PostgreSQL is deferred. When a DB is available (Story 1.9 CI / local Docker): run `pnpm --filter @sommycomfort/db db:migrate` to create the initial migration for `audit_logs`, and add an integration test that actually connects (health → `db: "up"`). The 1.11 ephemeral-Postgres integration harness gates this.
